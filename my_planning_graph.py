...
class PlanningGraph():
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        HINT: The Action instance associated with an action node is accessible
        through the PgNode_a.action attribute. See the Action class
        documentation for details on accessing the effects or preconditions of
        an action.
        ...
        if ((node_a1.action.precond_pos == node_a2.action.effect_add) and (node_a1.action.precond_neg == node_a2.action.effect_rem)) or ((node_a2.action.precond_pos == node_a1.action.effect_add) and (node_a2.action.precond_neg == node_a1.action.effect_rem)):
            return True
        return False
        ...
        for parent1 in node_a1.parents:
            for parent2 in node_a2.parents:
                if parent1.is_mutex(parent2):
                    return True
        # if ((node_a1.action.precond_pos == node_a2.action.precond_neg) and (node_a1.action.precond_neg == node_a2.action.precond_pos)):
        #     return True
        return False
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            goal_pgns = PgNode_s(goal, True)
            for level in range(len(self.s_levels)):
                if goal_pgns in self.s_levels[level]:
                    level_sum += level
                    break
        return level_sum
